  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h4 className="font-black text-navy text-sm uppercase">Equipe de Operação ({members.length})</h4>
        <button onClick={fetchMembers} className="text-[10px] font-black text-primary uppercase border-b border-primary/20">Atualizar</button>
      </div>
      ...
      <div className="grid gap-3">
        {members.length === 0 ? (
          ...
          <div key={member.id} className="flex items-center justify-between p-4 bg-slate-50/50 rounded-2xl border border-slate-100/50 hover:bg-white hover:shadow-sm transition-all group">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 bg-navy rounded-xl flex items-center justify-center text-white text-[10px] font-black uppercase">
                {member.role === 'LIDER_SETOR' ? <HardHat className="w-5 h-5" /> : <ShieldCheck className="w-5 h-5" />}
              </div>
              <div>
                <p className="font-black text-navy text-xs uppercase">{member.full_name}</p>
